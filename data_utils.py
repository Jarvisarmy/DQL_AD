...
def load_slices_from_BraTS(data_root):
    ...
    for subject in train_subjects:
        ...
        volume = crop_3d_volume_to_size(volume, resize_size=(192,192,96),target_size=(192,192,96))
        if np.ptp(volume) == 0:
            volume = np.zeros_like(volume, dtype='uint8')
        else:
            volume = ((volume-volume.min())/(volume.max()-volume.min())*255).astype('uint8')
        mask =  crop_3d_volume_to_size(mask, resize_size=(192,192,96),target_size=(192,192,96))
...
def load_slices_for_3D(data_root):
    """
    ...
    for subject in val_subjects:
        volume_path = os.path.join(BraTS_image_path, subject+"_t2.nii.gz")
